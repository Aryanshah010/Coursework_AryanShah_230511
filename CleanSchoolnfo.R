...
# Load cleaned Postcode to LSOA dataset
postcode_lsoa <- read_csv("Cleaned Data/cleanPostcodeLSOA.csv") %>%
  select(Postcode, District, LSOA, County)
...
clean_ks4final <- function(year, ks4_file, school_info_file) {
  ...
  school_info_yorkshire <- school_info %>%
    rename(SchoolName = SCHNAME,
           ...
           Postcode = POSTCODE) %>%
    filter(LocalAuthority %in% yorkshire_las) %>%
    mutate(Postcode = toupper(gsub(" ", "", Postcode))) %>%  # Clean Postcode
    select(URN, SchoolName, LocalAuthority, Postcode) %>%
    mutate(County = if_else(LocalAuthority %in% south_yorks, "South Yorkshire", "West Yorkshire"))
  ...
  ks4_yorkshire <- ks4 %>%
    ...
    select(URN, SchoolName, SchoolName_ks4, LocalAuthority, County, Postcode, Attainment8) %>%
    mutate(SchoolName = coalesce(SchoolName, SchoolName_ks4)) %>%
    select(URN, SchoolName, LocalAuthority, County, Postcode, Attainment8) %>%
    mutate(Year = year)
  ...
  # Optionally join with postcode_lsoa to add LSOA and District
  ks4_yorkshire <- ks4_yorkshire %>%
